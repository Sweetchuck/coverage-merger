...
use Sweetchuck\CoverageMerger\PhpMergerXmlOut;
use Sweetchuck\CoverageMerger\Test\UnitTester;
use Symfony\Component\Console\Output\BufferedOutput;

/**
 * @covers \Sweetchuck\CoverageMerger\PhpMergerXmlOut<extended>
 */
class PhpMergerXmlOutTest extends Unit
{
    ...
    public function testMergePhpFiles()
    {
        ...
        $merger = new PhpMergerXmlOut();
        $merger->mergePhpFiles($coverageFiles, $output);

        $actualXml = $output->fetch();
        $actualDoc = new \DOMDocument();
        $actualDoc->loadXML($actualXml);
        $metricsElements = $actualDoc->getElementsByTagName('metrics');
        $this->tester->assertGreaterOrEquals(1, $metricsElements->length);
    }
}
